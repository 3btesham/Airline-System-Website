...
def airline_staff():
    try:
        ...
        # print(output2)
        return render_template('airline_staff_dashboard.html', staff = user_data[0], airline_name = airline_name, future_flights = output2)
...
def add_flight():
    if request.method == 'POST':
        try:
            ...
            # first, run a query to check if this flight alreay exists
            query = """
            SELECT *
            FROM Flight
            WHERE airline_name = %s, flight_number = %s, depart_date = %s, depart_time = %s
            """
            cursor.execute(query, (airline_name, flight_number, depart_date, depart_time))
            output = cursor.fetchall()
            
            if output: # flight found, cannot add to database
